...
5.	We connect databricks compute with blob storage using service principal and Azure Key vault services. For that, Register the Service Principal also referred to as Azure AD Application
...
7.	Now, create a secret for the Service Principal and assign the Storage Blob Data Contributor role on the Data Lake for the Service Principal that gives full access to the storage account.
...
9.	Create a Azure key vault service named **‘formula1-kv-keys’** and create 3 secrets i.e clientid, tenanted, clientsecret taken from azure service principal.
10.	After that create a scope for the databricks to link it with the azure key vault.
11.	Create a new notebook in databricks in set-up folder **mount_adls_storage**. This contains the code to coonect to the datalake storage using service principal accessing secrets through azure key vault. Now mount all 3 containers onto databricks in the location **‘mnt/formula1dldatastorage’**
